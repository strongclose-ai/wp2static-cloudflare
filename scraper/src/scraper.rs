...
pub fn scrape_page(url: &str, base_url: &str, output_dir: &Path) -> Result<PathBuf> {
    ...
    let response = reqwest::blocking::get(url)
        .context(format!("Failed to fetch page from {}", url))?;
...
fn generate_output_path(url: &str, base_url: &str, output_dir: &Path) -> Result<PathBuf> {
    ...
    let _base_parsed = Url::parse(base_url)
        .context(format!("Failed to parse base URL: {}", base_url))?;
...
mod tests {
    ...
    fn test_generate_output_path_nested() {
        ...
        let path = generate_output_path(url, base_url, &output_dir).unwrap();
        assert_eq!(path, PathBuf::from("/tmp/output/blog/2024/my-post/index.md"));
    }
